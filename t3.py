...
current_turn = chess.WHITE  # Default turn to start with
...
async def start_game(ctx):
    global board, mode, player_color, current_turn
    if mode is None:
    ...
    current_turn = chess.WHITE  # Default turn to white
    color_text = "white" if player_color == chess.WHITE else "black"

    # Generate and send the board image
    generate_board_image()
    await ctx.send(f"New chess game started in `{mode}` mode! You are `{color_text}`.", file=discord.File("chessboard.png"))
...
async def make_move(ctx, move: str):
    ...
    if current_turn != (player_color if mode == 'ai' else chess.WHITE):
        await ctx.send("It's not your turn yet. Please wait for the other player.")
        return
    ...
    try:
        ...
        if move_obj in board.legal_moves:
            ...
            if mode == 'ai' and current_turn == chess.BLACK:
                await ai_move(ctx)
            else:
                await ctx.send(f"It's your turn to move! Use `/move <move>` to make a move.")
...
async def ai_move(ctx):
    ...
    try:
        ...
        current_turn = chess.WHITE  # AI makes a move, now it's the player's turn
